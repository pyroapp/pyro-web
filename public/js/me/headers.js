 ...
 * @param {*} channel 
 */
 function addChannelHeader(channelId) {
    const header = document.getElementById('channelHeaders');

    header.innerHTML += `
        <section class="title-3qD0b- container-1r6BKw themed-ANHk51 hidden header-fj033" id="header-${channelId}">
            <div class="children-19S4PO">
                ...
                <h3 class="cursorPointer-1j7DL8 title-29uC1r base-1x0h_U size16-1P40sf"></h3>
                <div aria-label="Offline" class="status-1XNdyw disableFlex-2QuzIB">
                    <svg width="10" height="10" viewBox="0 0 10 10" class="mask-1qbNWk">
                        <rect width="10" height="10" class="userStatus" fill="#43B581" mask="url(#svg-mask-status-online)"></rect>
                    </svg>
                </div>
                        ...
                        <div class="searchBar-3dMhjb">
                            <div class="DraftEditor-root">
                                <div class="public-DraftEditorPlaceholder-root">
                                    <div class="public-DraftEditorPlaceholder-inner" id="placeholder-5qjkp" style="white-space: pre-wrap;">Search</div>
                                </div>
                                <div class="DraftEditor-editorContainer">
                                    <div aria-describedby="placeholder-5qjkp" class="notranslate public-DraftEditor-content" contenteditable="true" role="textbox" spellcheck="false" style="outline: none; user-select: text; white-space: pre-wrap; overflow-wrap: break-word;">
                                        <div data-contents="true">
                                            <div data-block="true" data-editor="5qjkp" data-offset-key="c376h-0-0">
                                                <div data-offset-key="c376h-0-0" class="public-DraftStyleDefault-block public-DraftStyleDefault-ltr"><span data-offset-key="c376h-0-0"><br data-text="true"></span></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
 ...
 * @param {*} username 
 */
function setHeaderUsername(channelId, username) {
    const header = document.getElementById(`header-${channelId}`);

    header.querySelectorAll('.title-29uC1r')[0].innerText = username;
}


function setHeaderStatus(channelId, status) {
    if (!status) return;
